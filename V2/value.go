...
func StructField(itr interface{}, name string) []any {
	if name == "" {
		return nil
	}
	...
	return fields(itr, strings.Split(name, constant.Point))
}

func fields(data interface{}, fields []string) []any {
	var (
		value      = GetPtr(reflect.ValueOf(data))
		res        = make([]any, value.Len())
		idx, found = getIndex(
			getPtrStruct(value.Index(0)),
			make([]int, 0, len(fields)),
			fields)
		vi *reflect.Value
	)
	...
	for i := 0; i < value.Len(); i++ {
		vi = getValue(GetPtr(value.Index(i)), idx)
		//如果为空不做记录
		if vi == nil {
			continue
		}

		res[i] = vi.Interface() //添加当前值到对应位置
	}
	return res
}
